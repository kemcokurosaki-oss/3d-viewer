...
- [ ] 既存 Supabase データの扱い（SharePointへの再アップ・フォルダ整理は黒崎さんが対応予定。完了状況は次回確認すること）
- [x] Entra ID のクライアントID・テナントID → 確定済み。`sharepoint-client.js` に反映済み
- [x] 管理者による `Files.Read.All` への同意 → 付与済み・疎通確認済み
- [ ] `★3Dモデル` フォルダの編集権限設定 → **後回しと判断（意図的に保留）**。社内の人はこのフォルダの存在自体を認知していないため誤削除のリスクは低いと判断。必要になった時点で設定する（手順は会話履歴を参照）
- [x] `sharepoint-client.js` → Entra ID登録・疎通確認まで完了
- [x] `index.html` への本組み込み → 完了。アップロード・削除機能を撤去し「SharePointで開く」ボタンに置き換え、`viewer-data.js` 経由でSharePoint+Supabaseメタ情報を統合表示。**未実機検証**（コードレビューベースの実装。黒崎さんによる実際のブラウザ動作確認がまだ）
- [ ] 実機での動作確認（案件選択→ファイル一覧表示→プレビュー→サムネイル変更→表示名編集→並べ替え→SharePointで開く、の一連の流れ）
